...
bool gameScene::init()
{
	...
	if(tileMap->getObjectGroup("obstacle"))
	{
		...
		countNum = 0;
		checkObDirection = false; //false : ø¿∏•¬  true : øﬁ¬ 

		this->schedule(schedule_selector(gameScene::moveObstacleHeight), 1.0f); // øÚ¡˜¿Ã¥¬ ¿Âæ÷π∞ ±∏«ˆ
	}
	...
	__NotificationCenter::getInstance()->addObserver(this,
		...
		"notification", NULL);
	//"notification"¿Ã∂Û¥¬ ∏ﬁΩ√¡ˆ∞° ø¿∏È «ÿ¥Á «‘ºˆ∏¶ Ω««‡«—¥Ÿ.
...
void gameScene::createObstacle()
{
	...
	obstacle = Sprite::createWithTexture(obTexture, Rect(0, 0, 60, 60)); // ∏ ø° ∏¬√Á º˝¿⁄ πŸ≤„æﬂ«‘
...
void gameScene::moveCharacter(float dt)
{
	...
	if (playerPos.x <= (tileMap->getMapSize().width * tileMap->getTileSize().width)
		...
		&& playerPos.x >= 0 )
	{
		// ƒ≥∏Ø≈Õ∞° ¿Ãµø«“ ¿ßƒ°∞° ∏  æ»¿Œ∞ÊøÏ ∫Æø° √Êµπ«ﬁ¥¬¡ˆ∏¶ ∞ÀªÁ«’¥œ¥Ÿ By Daun
	...
	{
		// ƒ≥∏Ø≈Õ∞° ¿Ãµø«“ ¿ßƒ°∞° ∏  π€¿Ãπ«∑Œ ∫Æø° √Êµπ«— ∞Õ∞˙ ∏∂¬˘∞°¡ˆ¿‘¥œ¥Ÿ.
		checkCrash = CrashWithWall;
		moveDirection = (moveDirection + 2 ) % 4;
	}
	...
	if(checkCrash == CrashWithWall)
	{
		...
		{
			// ∫Æ∞˙ √Êµπ«— ∞ÊøÏ «ÿæﬂ«“ ¿œ
			character->setPosition(originalplayerPos);							// By Daun.. √Êµπ¿Œ ∞ÊøÏ ø¯∑° ¿ßƒ°∑Œ ∞ËΩÔ ¿Ø¡ˆ
			character_XP -= 10;

			int gaugeSize_part = 441/10;										// ∞‘¿Ã¡ˆπŸ ªÁ¿Ã¡Ó¿« 10∆€ºæ∆Æ ±Ê¿Ã
	...
	else
	{
		if (playerPos.x <= (tileMap->getMapSize().width * tileMap->getTileSize().width)
			...
			&& playerPos.x >= 0 )
		{
			character->setPosition( playerPos );							// ƒ≥∏Ø≈Õ¿« ªı∑ŒøÓ ¿ßƒ° ¡ˆ¡§
		}
	}
...
void gameScene::createCharacter()
{
	...
	//character->setFlipX(true); // X√‡ ±‚¡ÿ¿∏∑Œ π›¿¸
	//character->setFlipY(true);	// Y√‡ ±‚¡ÿ¿∏∑Œ π›¿¸
...
void gameScene::doNotification(Ref *obj)
{
	...
	if(flag==1)
	{
		Director::getInstance()->resume();														//»≠∏È ¿ÁΩ√¿€
	...
	{	
		...
		Director::getInstance()->pause();													//»≠∏È ¡§¡ˆ
		//Director::getInstance()->getTouchDispatcher()->removeDelegate(PauseMenu);			//∏ﬁ¥∫πˆ∆∞ ∫Ò»∞º∫
	}
...
void gameScene::check_item(float dt)
{
	...
	if(characterRect.intersectsRect(item4Rect))
	{
		...
		isSuper = true;	//π´¿˚ »∞º∫»≠
		count = 0;	//√ ºº±‚ √ ±‚»≠
		this->schedule(schedule_selector(gameScene::countTime),1.0f);	//√ ºº±‚ Ω√¿€
		this->doParticle();
...
void gameScene::decreaseGaugeBar(int num)
{
	...
	if( character_XP > 0 )
	{
		gaugeHeart->setPositionX(size.width - (20 + num));		// num ∞™ ∏∏≈≠ ∞®º“Ω√≈¥.	
	}
	else
	{
		//∞‘¿”¿ª ≥°≥Ω¥Ÿ
...
void gameScene::increaseGaugeBar(int num)
{
	...
	{
		gaugeHeart->setPositionX(size.width - (20 - num));		// num ∞™ ∏∏≈≠ ¡ı∞°Ω√≈¥.	
	}
	else
	{
		//∞‘¿”¿ª ≥°≥Ω¥Ÿ
